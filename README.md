...
**Оверлей статов CS2/CS:GO для OBS** — счётчик килов, смертей и K/D в реальном времени на стриме. Данные идут из игры через [Game State Integration (GSI)](https://developer.valvesoftware.com/wiki/Counter-Strike:_Global_Offensive_Game_State_Integration).

---

## Что это и кому полезно

- **Что делает:** поднимает маленький HTTP-сервер, который принимает данные от CS2/CS:GO и отдаёт страницу-оверлей. В OBS добавляешь источник «Браузер» с URL этого оверлея — на стриме отображаются твои килы, смерти и K/D. Статы обновляются по ходу матча и обнуляются при смене карты.
- **Кому подойдёт:** стримерам и записывающим геймплей в CS2/CS:GO, кто хочет показывать свои килы/смерти/K/D поверх видео без ручного ввода. Работает при игре на том же ПК, что OBS, или когда сервер в виртуалке (Hyper-V и т.п.) в одной сети с хостом.
- **Куда копировать код:**  
...
Так настроено у меня: сервер крутится на Linux в VM Hyper-V, игра и OBS — на Windows-компе.
...
**Только твои статы:** оверлей показывает килы/смерти/K/D только локального игрока. В режиме спектатора (наблюдение за тиммейтом) статы не переключаются на наблюдаемого — остаются твои. Сервер запоминает твой SteamID при первом POST от игры и игнорирует обновления `player`, когда приходит другой steamid.
